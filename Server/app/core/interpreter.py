...
class OperationInterpreter:
    ...
    def _load_modules_v1(self):
        ...
        try:
            ...
            from filewarp.core.pdf.core import PageExtractor, PDFCombine, PDF2LongImageConverter
            from filewarp.core.audio.core import AudioConverter, AudioExtracter, AudioJoiner
            from filewarp.core.video.core import VideoConverter
            from filewarp.core.image.core import ImageConverter, ImageCompressor, ImagePdfConverter
    ...
    def _load_modules(self):
        ...
        module_imports = {
            ...
            'video': [
                ('filewarp.core.video.core', 'VideoConverter')
            ],
            'image': [
            ...
            'ocr': [
                ('filewarp.core.ocr', 'ExtractText')
            ],
            'svg': [
                ('filewarp.core.svg.core', 'SVGConverter')
            ],
            'text': [
                ('filewarp.core.text.core', 'StyledText')
            ],
            'html': [
                ('filewarp.core.html', 'HTML2Word')
            ],
            'recorder': [
                ('filewarp.core.recorder', 'SoundRecorder')
            ],
            'voice': [
                ('voice.VoiceType', 'VoiceTypeEngine')
            ],
            'utils': [
                ('filewarp.utils.file_utils', 'generate_filename')
            ]
        }
    ...
    def _handle_doc_conversion(self, params: Dict[str, Any], progress_callback=None):
        ...
        for i, path in enumerate(input_paths):
            if progress_callback:
                progress_callback(int((i/total)*100), f"Processing {Path(path).name}")
    ...
    def _handle_html_to_word(self, params: Dict[str, Any], progress_callback=None):
        ...
        for i, html_file in enumerate(input_paths):
            if progress_callback:
                progress_callback(int((i/len(input_paths))*100), f"Converting {Path(html_file).name}")

            output = generate_filename(ext="docx", basedir=Path(html_file)) if generate_filename else None
            result = converter.convert_file(html_file, output)
            results.append({'input': html_file, 'output': str(output) if output else None})
    ...
    def _handle_audio_conversion(self, params: Dict[str, Any], progress_callback=None):
        ...
        if progress_callback:
            progress_callback(20, f"Converting {Path(input_path).name} to {target_format}")
    ...
    def _handle_image_conversion(self, params: Dict[str, Any], progress_callback=None):
        ...
        if progress_callback:
            progress_callback(30, f"Converting {Path(input_path).name} to {target_format}")
    ...
    def _handle_images_to_grayscale(self, params: Dict[str, Any], progress_callback=None):
        ...
        converter = GrayscaleConverter(input_paths[0] if len(input_paths) == 1 else input_paths)
        result = converter.run()
    ...
    def _handle_extract_pages(self, params: Dict[str, Any], progress_callback=None):
        ...
        if progress_callback:
            progress_callback(20, f"Extracting pages {pages} from {Path(input_path).name}")
    ...
    def _handle_svg_conversion(self, params: Dict[str, Any], progress_callback=None):
        ...
        output = generate_filename(ext=target_format, basedir=Path(input_path)) if generate_filename else None
